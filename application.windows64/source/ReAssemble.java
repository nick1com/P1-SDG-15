...
public void River(int itemp) {

  int i = itemp;
  image(background1Ext3, i*width-moveClo, 0+30);
}

public void RiverBack(int itemp) {
...
class Characters {
  ...
  public void moveMent(int minRtemp, int maxRtemp, int minLtemp, int maxLtemp, int minJTemp, int maxJTemp) {
    int minR =minRtemp;
    int minL = minLtemp;
    int maxR = maxRtemp;
    int maxL = maxLtemp;
...
//    moveJump(0, width*15);
  ...
  public void moveJump(int minTemp, int maxTemp) {
    int Max = maxTemp;
    int Min = minTemp;
//println(personYOrig);
//println(personY);
  ...
  public void landing(float distY) {
    float newY=distY;
    personY=-newY;
    groundLevel=true;
